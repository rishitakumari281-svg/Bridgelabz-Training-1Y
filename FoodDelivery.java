class Order implements Runnable {
    private int orderId;
    private String restaurant;
    private int deliveryTime;

    public Order(int orderId, String restaurant, int deliveryTime) {
        this.orderId = orderId;
        this.restaurant = restaurant;
        this.deliveryTime = deliveryTime;
    }

    public void run() {
        long start = System.currentTimeMillis();

        System.out.println("Order " + orderId + " from " + restaurant + " picked up by "
                + Thread.currentThread().getName());

        try {
            Thread.sleep(deliveryTime * 500);
            System.out.println("Order " + orderId + " is in transit by "
                    + Thread.currentThread().getName());

            Thread.sleep(deliveryTime * 500);
        } catch (InterruptedException e) {
            System.out.println("Order interrupted");
        }

        long end = System.currentTimeMillis();

        System.out.println("Order " + orderId + " delivered by "
                + Thread.currentThread().getName() + " | Time: "
                + (end - start) + " ms");
    }
}

public class FoodDelivery {
    public static void main(String[] args) {

        Thread o1 = new Thread(new Order(1, "KFC", 4), "Agent-1");
        Thread o2 = new Thread(new Order(2, "Dominos", 3), "Agent-2");
        Thread o3 = new Thread(new Order(3, "Burger King", 5), "Agent-3");
        Thread o4 = new Thread(new Order(4, "Pizza Hut", 2), "Agent-4");
        Thread o5 = new Thread(new Order(5, "Subway", 4), "Agent-5");

        o1.setPriority(10);
        o2.setPriority(5);
        o3.setPriority(3);
        o4.setPriority(10);
        o5.setPriority(5);

        o1.start();
        o2.start();
        o3.start();
        o4.start();
        o5.start();
    }
}